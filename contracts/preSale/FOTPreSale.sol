...
contract FOTPreSale is ReentrancyGuard, IFOTPreSale {
    ...
    * @param _roundID roundId
    ...
    function buyTokenByStableCoin(
        ...
        console.log("APPROVED AMOUNT FROM CONTRACT ===== ",totalCost);
        // Validate allowance
        require(
            IERC20(tokenAddress).allowance(msg.sender, address(this)) >=
                totalCost,
    ...
    /**
     * @dev Purchases FOT with fiat payment
     * @param _fotAmount Number of FOT tokens to purchase
        ...
        notPaused
    {
        ...
        // Emit event
        emit PurchasedByFiat(_beneficiary, _fotAmount);
    ...
    function _processPurchase(uint256 _fotAmount, address _buyer) private {
        ...
        if (buyCap > 0) {
            ...
            require(
                ...
                "FOTPreSale::You've reached the daily buying limit"
            );
        ...
        bool success = fotVesting.createVesting(
            _buyer,
    ...
    function _validateAmount(uint256 _fotAmount) private view {
        ...
        require(
            fotSold + _fotAmount <= fotLimit,
            "PreSale::FOT limit Exceeded"
        );

        // Purchase amount exceeds minimum
        require(
            _fotAmount >= minFotPerUser,
            "PreSale::Insufficient amount:Below minFOTPerUser"
        );
    }
    ...
    function _validateUserBalanceLimit(
        ...
        require(
            oldFotValue + _fotvalue <= userBalanceLimit,
            "PreSale::You've reached the max FOT amount"
        );
    }
